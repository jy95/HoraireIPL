...
trait jacop {
  ...
  def IntVar(name: String, min: Int, max: Int) = {
    new IntVar(name, min, max);
  }

  def BoolVar(name: String) = {
    new BoolVar(name);
  }
  ...
  implicit def IntVarToBoolVar(i:IntVar):BoolVar = {
    val v=new BoolVar(i.id())
    v <=> (i#=1)
    v
  }
  
  implicit def BoolVarToIntVar(b:BoolVar):IntVar = {
    val v=new IntVar(b.id(),0,1)
    b <=> (v#=1);
    v
  }

  implicit def listIntVarToArrayIntVar(li: List[IntVar]): Array[IntVar] = {
    li.toArray
  }

  implicit def listIntToArrayInt(li: List[Int]): Array[Int] = {
    li.toArray
  }

  implicit def listListIntVarToArrayArrayIntVar(li: List[List[IntVar]]): Array[Array[IntVar]] = {
    for (i <- Array.range(0, li.length)) yield li(i).toArray
  }
...
class IntSet extends org.jacop.core.IntervalDomain {
   ...
   */
  def unary_~ : IntSet = {
    var tmp = new IntSet(org.jacop.core.IntDomain.MinInt, org.jacop.core.IntDomain.MaxInt)
...
class BoolVar(name: String, min1: Int, max1: Int) extends org.jacop.core.BooleanVar(Model, name, min1, max1)
  ...
  def xor(that: org.jacop.core.IntVar) = {
    val result = new BoolVar()
    val c = new org.jacop.constraints.XorBool(Array(this, that),result)
    Model.constr += c
    result
  }
